import $ from 'jquery'
import '../assets/js/vendor/jquery.meanmenu'
import 'jquery-nice-select';
import 'jquery-nice-select/css/nice-select.css'; // обязательно подключи стили
...
export function initMeanMenu() {
    $('#mobile-menu').each(function () {
        const $this = $(this);
        const $screenWidth = $this.attr('data-screen') ? parseInt($this.attr('data-screen'), 10) : 991;

        if (!$this.hasClass('meanmenu-initialized')) {
            $this.addClass('meanmenu-initialized');
            $this.meanmenu({
